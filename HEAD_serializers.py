...
def _resolver_cantidad_por_caja(attrs):
    ...
    millar, etc.) ÔÇö opci├│n A gen├®rica de empaque.
...
def _validar_almacen_forzado(self, objeto, campo):
    """
    valida el almac├®n ANTES de guardar, en vez de crear el
    Movimiento y reci├®n despu├®s comprobar con check_almacen_objeto (patr├│n
    viejo: rollback post-save en la vista). 'objeto' es un Material o una
    Pieza ÔÇö si es Pieza, se valida el almac├®n de su propio material (el
    material real de ESA pieza, no el de un eventual contenedor).
    'self.context["almacen_forzado"]' lo pasa la vista: es None para
    Administrador, y el id del almac├®n asignado para Almacenero/Inspector.
    """
    ...
    if almacen_id != almacen_forzado:
        raise serializers.ValidationError({
            campo: "No puedes registrar movimientos fuera de tu almac├®n asignado."
        })
