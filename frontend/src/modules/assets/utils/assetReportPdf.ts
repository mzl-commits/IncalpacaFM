...
export async function generateAssetApaPdf({
  ...
  // JERARQUÍA DE 9 NIVELES SEGÚN ESPECIFICACIÓN
  ...
  const n8Code = (payload.n8_code as string) || (payload.piece_code as string) || "6A";
  const n8Name = (payload.piece as string) || (payload.pieceName as string) || "Garrucha (Rueda de Nylon)";

  const rawSku = (payload.n9_code as string) || (payload.sku as string) || (payload.skuCode as string) || asset.fm_sequence_value || "10";
  const skuNum = String(rawSku).replace(/^SKU\s*/i, "").trim();
  const n9Code = `SKU${skuNum}`;

  // CÓDIGO DE TAXONOMÍA CONCATENADO DE 9 NIVELES
  const fullMatrixCode = `${n1Code}-${n2Code}-${n3Code}-${n4Code}-${n5Code}-${n6Code}-${n7Code}-${n8Code}-${n9Code}`;

  // IDENTIFICADOR TÉCNICO ÚNICO
  const technicalId = asset.code || `INC-BIEN-2026-000215`;
  ...
  const responsibleRows = (asset.responsible_history?.length ?? 0) > 0
    ? asset.responsible_history.map(r => `
        <tr>
          ...
          <td>${r.reason || "Asignación técnica vigente"}</td>
        </tr>`).join("")
...
<div class="main-report">
  ...
  <div class="page-header">
    ...
    <div class="header-right">
      ...
      <span>ID Técnico Único: <strong>${technicalId}</strong></span><br/>
      <span>Código Taxonomía: <strong>${fullMatrixCode}</strong></span>
    </div>
  </div>
  ...
  <div class="section-block">
    ...
    <table class="data-table">
      <tbody>
        <tr>
          <td class="label" style="width:24%">Nivel 1 (Sede):</td>
          <td class="value" style="width:26%"><strong>[${n1Code}]</strong> ${n1Name}</td>
          <td class="label" style="width:24%">Nivel 2 (Área Macro):</td>
          <td class="value" style="width:26%"><strong>[${n2Code}]</strong> ${n2Name}</td>
        </tr>
        <tr>
          <td class="label">Nivel 3 (Área):</td>
          <td class="value"><strong>[${n3Code}]</strong> ${n3Name}</td>
          <td class="label">Nivel 4 (Módulo):</td>
          <td class="value"><strong>[${n4Code}]</strong> ${n4Name}</td>
        </tr>
        <tr>
          <td class="label">Nivel 5 (Tipo de Bien):</td>
          <td class="value"><strong>[${n5Code}]</strong> ${n5Name}</td>
          <td class="label">Nivel 6 (Bien):</td>
          <td class="value"><strong>[${n6Code}]</strong> ${n6Name}</td>
        </tr>
        <tr>
          <td class="label">Nivel 7 (Característica):</td>
          <td class="value"><strong>[${n7Code}]</strong> ${n7Name}</td>
          <td class="label">Nivel 8 (Variante/Modelo):</td>
  ...
  <div class="section-block">
    ...
    <table class="data-table">
      <tbody>
        <tr>
          <td class="label" style="width:24%">ID Técnico Único:</td>
          <td class="value" style="width:26%"><strong>${technicalId}</strong></td>
          <td class="label" style="width:24%">1. Código de Trabajador:</td>
          <td class="value" style="width:26%"><strong>${workerCode}</strong></td>
        </tr>
        <tr>
  ...
  <!-- SECCIÓN 7: TRAZABILIDAD DIGITAL Y QR -->
  <div class="section-block">
    <div class="section-heading">7. CÓDIGO DE VERIFICACIÓN Y TRAZABILIDAD DIGITAL</div>
    <div class="qr-block">
      <div class="qr-cell">
        <img src="${qrDataUrl}" alt="QR vinculado a ${technicalId}">
        <span>Escanear para verificar ID Técnico Único</span>
      </div>
      <div class="qr-info">
        <table class="data-table">
          <tbody>
            <tr>
              <td class="label" style="width:35%">ID Técnico Único (QR):</td>
              <td class="value"><strong>${technicalId}</strong></td>
            </tr>
            <tr>
              <td class="label" style="width:35%">Código Taxonomía (9 Niveles):</td>
              <td class="value"><strong>${fullMatrixCode}</strong></td>
            </tr>
            <tr>
              <td class="label" style="width:35%">URL de Verificación:</td>
              <td class="value">${publicUrl}</td>
            </tr>
            <tr>
              <td class="label" style="width:35%">Normas:</td>
