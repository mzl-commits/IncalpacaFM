...
export function AssetMapOverviewPage() {
  ...
  const filteredLocations = useMemo(() => locations.filter((item) => {
    const matchesImage = imageFilter === "ALL" || (imageFilter === "WITH_MAP" ? item.activeMap : !item.activeMap);
    const haystack = `${item.locationCode} ${item.zone} ${item.building} ${item.area} ${item.room}`.toLocaleLowerCase("es-PE");
  ...
  const scopedLocations = searchMode ? filteredLocations : filteredLocations.filter((item) =>
    (!zone || item.zone === zone) && (!building || item.building === building) && (!area || item.area === area),
  );
  ...
  const levelTitle = searchMode ? `Resultados para “${query.trim()}”` : area ? area : building ? building : zone ? zone : "Toda la planta";
  const levelDescription = searchMode
    ...
    : area ? "Selecciona un ambiente para consultar sus bienes y referencia visual."
    : building ? "La cuadrícula separa los ambientes por área funcional."
    : zone ? "Selecciona un edificio o agrupación operativa."
  ...
  return (
    <section className="asset-map-overview-page">
      <div className="page-heading asset-map-overview-heading">
        <div><p className="breadcrumb">Bienes / Explorador de ambientes</p><h1>Mapa administrativo</h1><p>Ubicaciones organizadas por taxonomía espacial. Las imágenes se usan únicamente como referencia dentro de cada ambiente.</p></div>
        <Link className="button button-secondary" to="/administracion/mapas-ambientes">Gestionar imágenes <ArrowSquareOut /></Link>
      </div>

      {!ready ? <div className="asset-map-overview-loading" aria-busy="true"><span className="sr-only">Cargando ambientes</span></div>
      : hasError ? <div className="taxonomy-state-panel" role="alert"><WarningCircle size={32} /><strong>No se pudo cargar el catálogo de ambientes</strong><p>Verifica la conexión e inténtalo nuevamente.</p></div>
      : <>
        <div className="asset-map-toolbar">
          <label className="asset-map-search"><MagnifyingGlass /><span className="sr-only">Buscar ambiente</span><input value={query} onChange={(event) => { setQuery(event.target.value); setSelectedLocationId(""); }} placeholder="Buscar código, zona, edificio o ambiente" /></label>
          <label><span>Referencia visual</span><select value={imageFilter} onChange={(event) => { setImageFilter(event.target.value as ImageFilter); setSelectedLocationId(""); }}><option value="ALL">Todos los ambientes</option><option value="WITH_MAP">Con imagen</option><option value="WITHOUT_MAP">Pendientes de imagen</option></select></label>
          <div className="asset-map-toolbar-summary"><span><Package /> {assets.length} bienes</span><span><Buildings /> {locations.length} ambientes</span><span><ImageSquare /> {mappedCount} con imagen</span></div>
        </div>

        <nav className="asset-map-drill-path" aria-label="Ruta de ubicación">
          <button type="button" onClick={resetToRoot} aria-current={!zone && !searchMode ? "page" : undefined}>Toda la planta</button>
          {!searchMode && zone && <><CaretRight /><button type="button" onClick={() => openZone(zone)} aria-current={!building ? "page" : undefined}>{zone}</button></>}
          {!searchMode && building && <><CaretRight /><button type="button" onClick={() => openBuilding(building)} aria-current={!area ? "page" : undefined}>{building}</button></>}
        ...
        <div className={`asset-map-grid-layout ${selectedLocation ? "has-inspector" : ""}`}>
          <main className="asset-map-grid-panel">
            <header><div><span className="asset-map-level-icon"><SquaresFour weight="duotone" /></span><div><h2>{levelTitle}</h2><p>{levelDescription}</p></div></div><span>{environmentLabel(scopedLocations.length)}</span></header>
            {searchMode || area ? <EnvironmentGrid locations={scopedLocations} assetsByLocation={assetsByLocation} selectedId={selectedLocationId} onSelect={setSelectedLocationId} />
            : building ? <GroupGrid values={unique(scopedLocations.map((item) => item.area))} locations={scopedLocations} assetsByLocation={assetsByLocation} label="área" onOpen={openArea} />
            : zone ? <GroupGrid values={unique(scopedLocations.map((item) => item.building))} locations={scopedLocations} assetsByLocation={assetsByLocation} label="edificio" onOpen={openBuilding} />
...
function GroupGrid({ values, locations, assetsByLocation, label, onOpen }: { values: string[]; locations: LocationOption[]; assetsByLocation: Map<string, unknown[]>; label: string; onOpen: (value: string) => void }) {
  if (!values.length) return <EmptyGrid />;
  return <div className="asset-map-taxonomy-grid">{values.map((value) => {
    const children = locations.filter((item) => label === "zona" ? item.zone === value : label === "edificio" ? item.building === value : item.area === value);
    ...
    return <button type="button" className="asset-map-group-tile" key={value} onClick={() => onOpen(value)}><span className="asset-map-tile-icon"><Buildings weight="duotone" /></span><span className="asset-map-tile-copy"><strong>{value}</strong><small>{environmentLabel(children.length)} · {assets} bienes</small></span><span className="asset-map-tile-coverage"><ImageSquare /> {mapped}/{children.length}</span><CaretRight /></button>;
  })}</div>;
}

function EnvironmentGrid({ locations, assetsByLocation, selectedId, onSelect }: { locations: LocationOption[]; assetsByLocation: Map<string, unknown[]>; selectedId: string; onSelect: (id: string) => void }) {
...
function EmptyGrid() {
  return <div className="asset-map-overview-empty"><MagnifyingGlass /><strong>No encontramos ambientes</strong><p>Ajusta la búsqueda o cambia el filtro de imagen referencial.</p></div>;
}
