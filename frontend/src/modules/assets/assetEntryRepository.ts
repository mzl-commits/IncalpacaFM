...
function mapAsset(item: AssetApiRecord): RegisteredAsset {
  return {
    ...
    draft: { ...item.entry_payload, name: item.name, entryType: item.entry_type },
  };
}
...
export async function saveAssetEntryDraft(draft: AssetEntryDraft) {
  await offlineDb.drafts.put({
    id: DRAFT_ID, domain: "assets", payload: draft, updatedAt: new Date().toISOString(),
...
export async function getPublicAsset(token: string) {
  ...
  return data as {
    code: string; name: string; brand: string; model: string; condition: string;
    administrative_status: string; operational_status: string; classification: string;
    general_location: string; updated_at: string;
  };
}

export async function registerAsset(draft: AssetEntryDraft): Promise<RegisteredAsset> {
  const { data } = await api.post<AssetApiRecord>("/assets/", {
