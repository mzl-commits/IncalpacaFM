...
export function UserBetaProfilePage() {
  ...
  if (profileQuery.isError || !profileQuery.data) return <section className="user-profile-page"><div className="dashboard-partial-error" role="alert">No fue posible cargar este perfil. Verifica tus permisos e inténtalo nuevamente.</div><Link className="button button-secondary" to="/mapa-bienes"><ArrowLeft /> Volver a ambientes</Link></section>;

  const { profile, assigned_assets: assets } = profileQuery.data;
  return <section className="user-profile-page">
    <header className="page-heading user-profile-heading"><div><p className="breadcrumb">Ambientes / Personas</p><h1>{profile.display_name}</h1><p>Bienes activos bajo su responsabilidad y ambiente de referencia.</p></div><Link className="button button-secondary" to="/mapa-bienes"><ArrowLeft /> Volver a ambientes</Link></header>
