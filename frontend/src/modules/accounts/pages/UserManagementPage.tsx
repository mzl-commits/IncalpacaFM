...
export function UserManagementPage() {
  ...
  async function load() {
    ...
    catch { setError("No se pudo cargar el registro de usuarios. Verifica que el backend esté actualizado y vuelve a intentarlo."); }
  ...
  function update<K extends keyof TechnicianInput>(field: K, value: TechnicianInput[K]) { setForm((current) => ({ ...current, [field]: value })); }
  function openCreate() { setEditing(null); setForm(emptyUser); setError(""); setFeedback(""); }
  function openEdit(user: Technician) { setEditing(user); setForm({ ...user, temporary_password: "" }); setError(""); setFeedback(""); }
  ...
  async function save(event: React.FormEvent<HTMLFormElement>) {
    ...
    setSaving(true);
    const payload = { ...form, full_name: form.full_name.trim(), worker_code: form.worker_code.trim().toUpperCase(), dni: form.dni.replace(/\D/g, "") };
    try {
  ...
  return <section className="user-management-page">
    ...
    <div className="user-management-summary" aria-label="Resumen de usuarios"><div><UsersThree size={21} /><span><strong>{users.length}</strong><small>Usuarios registrados</small></span></div><div><span><strong>{activeCount}</strong><small>Con acceso activo</small></span></div><div><span><strong>{users.filter((user) => user.role === "SOLICITANTE").length}</strong><small>Solicitantes</small></span></div></div>
    {editing !== undefined && <form className="data-panel user-management-editor" onSubmit={save}><header><div><h2>{editing ? "Editar usuario" : "Nuevo usuario"}</h2><p>Los cambios se aplican al próximo inicio de sesión.</p></div><button type="button" className="icon-button" onClick={() => setEditing(undefined)} aria-label="Cerrar formulario"><X size={20} /></button></header><div className="form-grid"><label className="field field-wide"><span>Nombre completo *</span><input required value={form.full_name} onChange={(event) => update("full_name", event.target.value)} /></label><label className="field"><span>Código de trabajador *</span><input required maxLength={40} value={form.worker_code} onChange={(event) => update("worker_code", event.target.value.toUpperCase())} /></label><label className="field"><span>DNI *</span><input required inputMode="numeric" maxLength={8} value={form.dni} onChange={(event) => update("dni", event.target.value.replace(/\D/g, "").slice(0, 8))} /></label><label className="field"><span>Correo electrónico</span><input type="email" value={form.email} onChange={(event) => update("email", event.target.value)} /></label><label className="field"><span>Rol *</span><select value={form.role} onChange={(event) => update("role", event.target.value as UserRole)}>{roleOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label><label className="field"><span>{editing ? "Nueva contraseña temporal" : "Contraseña temporal *"}</span><input required={!editing} type="password" minLength={10} value={form.temporary_password} onChange={(event) => update("temporary_password", event.target.value)} placeholder={editing ? "Déjalo vacío para conservarla" : "Mínimo 10 caracteres"} /></label><label className="field"><span>Cargo</span><input value={form.position} onChange={(event) => update("position", event.target.value)} /></label><label className="field"><span>Especialidad o área</span><input value={form.specialty} onChange={(event) => update("specialty", event.target.value)} /></label><label className="field"><span>Tarifa por hora (S/)</span><input type="number" min="0" step="0.01" disabled={!technicalRole} value={form.hourly_rate} onChange={(event) => update("hourly_rate", Number(event.target.value) || 0)} /></label><label className="field field-toggle"><input type="checkbox" checked={form.active} onChange={(event) => update("active", event.target.checked)} /><span>Cuenta activa</span></label></div><footer className="form-actions"><button className="button button-secondary" type="button" onClick={() => setEditing(undefined)}>Cancelar</button><button className="button button-primary" disabled={saving} type="submit"><FloppyDisk size={18} />{saving ? "Guardando…" : "Guardar usuario"}</button></footer></form>}
    <ListFilterPanel title="Directorio de usuarios" description="Busca una cuenta o limita el listado por rol." searchLabel="Buscar usuarios" searchPlaceholder="Nombre, DNI, código o cargo" searchValue={query} onSearchChange={setQuery} resultCount={visibleUsers.length} totalCount={users.length} activeFilters={roleFilter === "TODOS" ? [] : [{ key: "role", label: "Rol", value: roleName(roleFilter), onRemove: () => setRoleFilter("TODOS") }]} onClear={() => { setQuery(""); setRoleFilter("TODOS"); }}><FilterSelect label="Rol" value={roleFilter === "TODOS" ? "" : roleFilter} onChange={(value) => setRoleFilter((value as UserRole) || "TODOS")} options={roleOptions} allLabel="Todos los roles" /></ListFilterPanel>
    <article className="data-panel user-management-list">{loading ? <div className="loading-panel">Cargando usuarios…</div> : !visibleUsers.length ? <div className="empty-state user-management-empty"><UsersThree size={28} /><strong>{error ? "No fue posible obtener el directorio" : "No hay usuarios con esos filtros"}</strong><span>{error ? "Reintenta cuando el backend actualizado esté disponible." : "Cambia los filtros o crea el primer usuario."}</span></div> : <div className="responsive-table"><table><thead><tr><th>Usuario</th><th>Rol</th><th>Código / DNI</th><th>Área o cargo</th><th>Acceso</th><th aria-label="Acciones" /></tr></thead><tbody>{visibleUsers.map((user) => <tr key={user.id}><td><strong>{user.full_name}</strong><small>{user.email || "Sin correo"}</small></td><td><span className="status-badge">{roleName(user.role)}</span></td><td><strong>{user.worker_code}</strong><small>{user.dni || "Sin DNI"}</small></td><td>{user.position || user.specialty || "No registrado"}</td><td><span className={`status-badge ${user.active ? "" : "is-muted"}`}>{user.active ? "Activo" : "Inactivo"}</span></td><td><button className="table-action" type="button" onClick={() => openEdit(user)}><PencilSimple size={17} />Editar</button></td></tr>)}</tbody></table></div>}</article>
  </section>;
}
