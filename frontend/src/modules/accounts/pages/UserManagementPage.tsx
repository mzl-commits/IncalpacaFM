...
export function UserManagementPage() {
  ...
  return <section className="user-management-page">
    ...
    {editing !== undefined && <form className="data-panel user-management-editor" onSubmit={save}><header><div><h2>{editing ? "Editar usuario" : "Nuevo usuario"}</h2><p>Los cambios se aplican al próximo inicio de sesión.</p></div><button type="button" className="icon-button" onClick={() => setEditing(undefined)} aria-label="Cerrar formulario"><X size={20} /></button></header><div className="form-layout-split">
      <fieldset className="form-section">
        ...
        <div className="form-grid">
          ...
          <label className="field field-wide"><span>Correo electrónico</span><input type="email" value={form.email} onChange={(event) => update("email", event.target.value)} /></label>
        </div>
      </fieldset>
      
      <fieldset className="form-section">
        <legend>Perfil y Accesos</legend>
        <div className="form-grid">
          <label className="field"><span>Rol *</span><select value={form.role} onChange={(event) => update("role", event.target.value as UserRole)}>{roleOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
    ...
    <style>{`
      .form-layout-split { display: grid; gap: 32px; }
      .form-section { border: none; padding: 0; margin: 0; }
      .form-section legend { font-weight: 700; color: #111827; font-size: 15px; margin-bottom: 16px; padding-bottom: 8px; border-bottom: 1px solid #e5e7eb; width: 100%; }
      @media (min-width: 900px) {
        .form-layout-split { grid-template-columns: 1fr 1fr; gap: 40px; }
      }
    `}</style>
    <footer className="form-actions"><button className="button button-secondary" type="button" onClick={() => setEditing(undefined)}>Cancelar</button><button className="button button-primary" disabled={saving} type="submit"><FloppyDisk size={18} />{saving ? "Guardando…" : "Guardar usuario"}</button></footer></form>}
