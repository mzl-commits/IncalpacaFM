...
export function ProfilePage() {
  ...
  return <section className="profile-page">
    ...
    <div className="profile-layout">
      ...
      <section className="profile-details-card"><header><div><span className="profile-kicker">Información de cuenta</span><h2>Datos personales y laborales</h2></div><IdentificationBadge size={24} /></header><dl className="profile-details-grid"><div><dt>Código de trabajador</dt><dd>{user.workerCode || "No registrado"}</dd></div><div><dt>DNI</dt><dd>{user.dni || "No registrado"}</dd></div><div><dt>Correo</dt><dd>{user.email || "No registrado"}</dd></div><div><dt>Cargo / posición</dt><dd>{user.position || "No especificado"}</dd></div><div><dt>Especialidad</dt><dd>{user.specialtyId || "No especificada"}</dd></div>{user.role === "TECNICO" && <div><dt>Tarifa por hora</dt><dd>S/ {Number(user.hourlyRate || 0).toFixed(2)}</dd></div>}</dl></section>
