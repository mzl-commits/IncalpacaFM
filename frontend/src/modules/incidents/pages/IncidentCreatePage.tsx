...
function locationLabel(location: LocationOption) {
  return location.displayName || `${location.locationCode ? `${location.locationCode} - ` : ""}${location.room}`;
}
...
export function IncidentCreatePage() {
  ...
  const locationResults = useMemo(() => {
    const normalized = normalizeSearch(locationQuery.trim());
    if (selectedLocation) return [];
    if (!normalized) return locations.slice(0, 8);
    return locations.filter((item) => locationSearchText(item).includes(normalized)).slice(0, 8);
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!form.locationId || !form.requestType || form.description.trim().length < 10) {
      setError("Selecciona una ubicación oficial, completa el tipo de solicitud y describe la solicitud con al menos 10 caracteres.");
      return;
    }
    ...
    try {
      await createWorkRequest({
        ...
        evidence: form.photoName
          ? [{ id: createClientId("evidence"), name: form.photoName, mimeType: "image/*", size: 0 }]
          : [],
        status: "PENDIENTE",
      });
    ...
    <section className="incident-create-page">
      <div className="page-heading">
        <div>
          ...
          <p>Registra la solicitud, ubícala en el catálogo oficial y adjunta una evidencia para facilitar su evaluación.</p>
        </div>
        ...
        <div className="form-section">
          <div className="section-heading">
            <div>
              ...
              <div>
                ...
                <p>Busca por código, oficina, ambiente, área, edificio o ubicación específica. La plataforma completará la jerarquía automáticamente.</p>
              </div>
            </div>
          </div>
          ...
          <div className="incident-location-layout">
            <div className="incident-location-search">
              <label className="field">
                ...
                <div className="incident-location-combobox">
                  ...
                  <input
                    ...
                    aria-expanded={locationFocused && (locationResults.length > 0 || locationsQuery.isPending)}
                    ...
                    onKeyDown={(event) => {
                      ...
                      if (event.key === "ArrowDown") {
                        event.preventDefault();
                        setActiveResult((current) => Math.min(current + 1, locationResults.length - 1));
                      }
                      ...
                      if (event.key === "Enter") {
                        event.preventDefault();
                        selectLocation(locationResults[activeResult]);
                      }
              ...
              {locationFocused && !selectedLocation && (
                <div className="incident-location-results" id="incident-location-results" role="listbox">
                  {locationsQuery.isPending ? (
                    <p>Cargando ubicaciones oficiales...</p>
                  ) : locationResults.length ? (
                    locationResults.map((location, index) => (
                      <button
                        id={`incident-location-${location.id}`}
                        role="option"
                      ...
                      >
                        ...
                        <span>
                          ...
                          <small>{location.zone} / {location.building} / {location.area}</small>
                        </span>
                        {location.activeMap ? <em><ImageSquare weight="fill" /> Con imagen</em> : <em>Sin imagen</em>}
                      </button>
                    ))
                  ) : locationsQuery.isError ? (
                    <p>No se pudieron cargar las ubicaciones. Recarga la página e inténtalo nuevamente.</p>
                  ) : (
                    <p>No hay coincidencias. Prueba con otro código, ambiente, área o edificio.</p>
                  )}
                </div>
              )}
            ...
            {selectedLocation && (
              <dl className="incident-location-summary">
                <div><dt>Zona</dt><dd>{selectedLocation.zone}</dd></div>
                <div><dt>Edificio</dt><dd>{selectedLocation.building}</dd></div>
                <div><dt>Área</dt><dd>{selectedLocation.area}</dd></div>
                <div><dt>Ambiente</dt><dd>{selectedLocation.room}</dd></div>
              </dl>
            )}
          </div>
          ...
          <div className="form-grid">
            ...
            <label className="field">
              ...
              <select
                ...
                onChange={(event) => updateField("requesterPriority", event.target.value as RequestPriority)}
              >
            ...
            <label className="field field-wide">
              ...
              <textarea
                ...
                placeholder="Describe qué se necesita, desde cuándo y cualquier detalle importante."
                rows={5}
                maxLength={1000}
              />
