...
from apps.catalogo.models import Material, Pieza, UnidadMedida
...
class MovimientoSerializer(serializers.ModelSerializer):
    ...
    unidad_movimiento_nombre = serializers.CharField(source="unidad_movimiento.nombre", read_only=True, default=None)
    unidad_movimiento_abreviatura = serializers.CharField(source="unidad_movimiento.abreviatura", read_only=True, default=None)
    ...
    class Meta:
        ...
        fields = [
            ...
            "cantidad_cajas",
            "unidad_movimiento", "unidad_movimiento_nombre", "unidad_movimiento_abreviatura",
...
def _resolver_cantidad_por_caja(attrs):
    """
    Si se envi├│ 'cantidad_cajas', recalcula 'cantidad' (en unidades) del lado
    del servidor como cantidad_cajas * material.unidades_por_caja. Soporta
    cualquier tipo de manejo con requiere_multiplicador=True (caja, bolsa,
    paquete, kit/juego, docena, millar, etc.) ÔÇö opci├│n A gen├®rica de empaque.
    Devuelve attrs modificado in-place. Lanza ValidationError si el material
    no tiene unidades_por_caja configuradas o si su manejo es por unidad suelta.
    """
    cantidad_cajas = attrs.get("cantidad_cajas")
    if cantidad_cajas:
        material = attrs.get("material_id")
        if material:
            if not material.unidad_manejo.requiere_multiplicador:
            ...
            if not material.unidades_por_caja:
                raise serializers.ValidationError({
                    "cantidad_cajas": (
                        ...
                        f"por empaque ({material.unidad_manejo.nombre}). "
...
def _resolver_cantidad_por_unidad_movimiento(attrs):
    """
    Para materiales tipo Rollo (unidad_manejo.permite_conversion_unidad=True):
    el stock se guarda internamente en material.unidad_movimiento_base (ej.
    cm), pero el almacenero puede registrar el movimiento en otra unidad
    compatible de la misma familia (ej. m). Convierte 'cantidad_en_unidad_movimiento'
    a la unidad base del material y lo deja en attrs['cantidad'].
    """
    cantidad_unidad = attrs.get("cantidad_en_unidad_movimiento")
    unidad_movimiento = attrs.get("unidad_movimiento_id")
    if cantidad_unidad and unidad_movimiento:
        material = attrs.get("material_id")
        if material:
            if not material.unidad_manejo.permite_conversion_unidad:
                raise serializers.ValidationError({
                    "unidad_movimiento_id": (
                        f"'{material.nombre}' no permite elegir unidad al registrar "
                        f"movimientos; indica la cantidad directamente."
                    )
                })
            base = material.unidad_movimiento_base
            if not base:
                raise serializers.ValidationError({
                    "unidad_movimiento_id": (
                        f"'{material.nombre}' no tiene configurada su unidad base de stock. "
                        f"Ed├¡talo antes de registrar este movimiento."
                    )
                })
            if unidad_movimiento.familia != base.familia:
                raise serializers.ValidationError({
                    "unidad_movimiento_id": (
                        f"'{unidad_movimiento.nombre}' no es compatible con la unidad base "
                        f"de '{material.nombre}' ({base.nombre})."
                    )
                })
            cantidad_base = (cantidad_unidad * unidad_movimiento.factor_a_base) / base.factor_a_base
            cantidad_base_entera = int(cantidad_base)
            if cantidad_base_entera < 1:
                raise serializers.ValidationError({
                    "cantidad_en_unidad_movimiento": (
                        f"La cantidad indicada equivale a menos de 1 {base.abreviatura}; "
                        f"ingresa un valor mayor."
                    )
                })
            attrs["cantidad"] = cantidad_base_entera
    return attrs
...
class SalidaMaterialSerializer(serializers.Serializer):
    ...
    unidad_movimiento_id = serializers.PrimaryKeyRelatedField(
        queryset=UnidadMedida.objects.all(), required=False, allow_null=True, default=None,
    )
    cantidad_en_unidad_movimiento = serializers.DecimalField(
        max_digits=12, decimal_places=3, required=False, allow_null=True, default=None,
    )
    ...
    def validate(self, attrs):
        ...
        attrs = _resolver_cantidad_por_unidad_movimiento(attrs)
        if not attrs.get("cantidad"):
            raise serializers.ValidationError({"cantidad": "Indica la cantidad, la cantidad de cajas, o la cantidad en la unidad elegida."})
        return attrs
    ...
    def create(self, validated_data):
        return registrar_salida_material(
            ...
            unidad_movimiento=validated_data.get("unidad_movimiento_id"),
            cantidad_en_unidad_movimiento=validated_data.get("cantidad_en_unidad_movimiento"),
        )
...
# ÔöÇÔöÇÔöÇ Solicitudes de movimiento (flujo de aprobaci├│n) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
...
class SolicitudMovimientoSerializer(serializers.ModelSerializer):
    ...
    # Detalle espec├¡fico de la pieza individual (ej. "Martillo de Juan", "Estuche azul")
...
class SolicitudMovimientoCreateSerializer(serializers.ModelSerializer):
    """Serializer de escritura ÔÇö usado por el ALMACENERO al crear una solicitud."""
...
class AprobarSolicitudSerializer(serializers.Serializer):
    """Payload vac├¡o ÔÇö la aprobaci├│n no requiere datos adicionales del admin."""
    pass
...
# ÔöÇÔöÇÔöÇ Grupos de Solicitudes (Objetivo 1) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
...
class GrupoSolicitudItemInputSerializer(serializers.Serializer):
    """Representa un item individual dentro del payload de creaci├│n de un grupo."""
...
class GrupoSolicitudCreateSerializer(serializers.Serializer):
    ...
    items = serializers.ListField(
        ...
        allow_empty=False,
        help_text="Lista de materiales solicitados en este grupo (m├¡nimo 1).",
    )
...
class ResolverParcialItemSerializer(serializers.Serializer):
    """Representa la decisi├│n del administrador para un item individual del grupo."""
...
class ResolverParcialGrupoSerializer(serializers.Serializer):
    """Payload para aprobaci├│n/rechazo parcial de un grupo."""
    items = serializers.ListField(
        child=ResolverParcialItemSerializer(),
        allow_empty=False,
        help_text="Decisi├│n para cada item del grupo.",
    )
